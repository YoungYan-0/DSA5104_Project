...
WHERE listing.name = 'Nice room with superb city view' AND date = '2016-01-04';
...
UPDATE calendar
SET availability = 0
WHERE property = 10547346 AND date >= '2016-01-04' AND date <= '2016-01-06';

SELECT *
FROM listing JOIN calendar ON listing.lid = calendar.property
WHERE calendar.date = '2016-01-04' AND calendar.availability = 1;

SELECT *
FROM listing
WHERE listing.neighborhood = 'Downtown' AND listing.property_type = 'Apartment' AND listing.bedrooms = 1 AND listing.beds = 1 AND listing.bathrooms = 1 AND listing.accommodates = 2 AND listing.price <= 300 AND listing.review_scores_rating >= 4;
